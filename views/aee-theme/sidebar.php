<aside id="sidebar-libros">
	<h3 class="section-title-dropdown">Libros</h3>
	<div class="libro-item clearfix fClear">
		<img class="aLeft libro_portada" src="assets/img/libros/Libro-1-2012-1.jpg" width="150" alt="Libro-1-2012-1">
		<h4 class="libro-titulo">Los periódicos <br> y sus plazas</h4>
		<ul>
			<li>El Siglo de Torreón</li>
			<li>Diario de Yucatán</li>
			<li>El Imparcial</li>
			<li>La Crónica</li>
			<li>El Siglo de Durango</li>
			<li>La Frontera</li>
			<li>La i</li>
			<li>Internet, Vallas y Publicidad Móvil</li>
		</ul>
		<a class="btn btn-pfd fLeft" href="?descargar=libro1&source=xtc#libro1" name="libro1">Descargar</a>
	</div>
	<div class="libro-item clearfix fClear">
		<img class="aLeft libro_portada" src="assets/img/libros/Libro-3-2012-1.jpg" width="150" alt="Libro-3-2012-1">
		<h4 class="libro-titulo">Tarifas <br> y medidas</h4>
		<ul>
			<li>El Siglo de Torreón</li>
			<li>Diario de Yucatán</li>
			<li>El Imparcial</li>
			<li>La Crónica</li>
			<li>El Siglo de Durango</li>
			<li>La Frontera</li>
			<li>La i</li>
			<li>Internet, Vallas y Publicidad Móvil</li>
		</ul>
		<a class="btn btn-pfd fLeft" href="?descargar=libro2&source=xtc#libro2" name="libro2">Descargar</a>
	</div>
</aside>